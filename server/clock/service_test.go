...
func TestService(t *testing.T) {
	...
	if s.GetAbsoluteTick() != 1 {
		t.Error("Absolute tick not incremented")
	}
	if s.GetRelativeTick() != 1 {
		t.Error("Relative tick not incremented")
	}
	...
	if s.GetAbsoluteTick() != 1+message.MaxTick {
		t.Error("Invalid absolute tick after message.MaxTick ticks")
	}
	if s.GetRelativeTick() != 1 {
		t.Error("Invalid relative tick after message.MaxTick ticks")
	}
}
